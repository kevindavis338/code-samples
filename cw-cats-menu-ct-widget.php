...
/*
Plugin Name: Cancer Wellness Category Listing for the Mega Menu by Cancer Type
...
function cw_cats_load_menu_ct_widget() {
    register_widget( 'cw_cats_menu_ct_widget' );
}
add_action( 'widgets_init', 'cw_cats_load_menu_ct_widget' );

// Creating the cw categories widget 
class cw_cats_menu_ct_widget extends WP_Widget {
 
	function __construct() {
		parent::__construct(
		
		// Base ID of your widget
		'cw_cats_menu_ct_widget', 
		
		// Widget name will appear in UI
		__('Cancer Wellness Categories for the Cancer Type', 'cw_cats_widget_domain'), 
		
		// Widget description
		array( 'description' => __( 'The widget for the to show the category in the Mega Menu by Cancer Type', 'cw_cats_widget_domain' ), ) 
		);
	}
				...
				foreach ($categories as $cat) {
					...
					if($cat->category_parent == 310 ) :
						...
						if (have_posts()) : while (have_posts()) : the_post(); ?>
							...
							<li><a href="<?php the_permalink();?>" title="<?php the_title(); ?>" class='limited-text'><?php the_title(); ?></a></li>
